...
class Mesh24 {
  public:
    ...
    inline byte getChannel() const {
      return channel;
    }

    inline void setSubnet(mesh24_subnet_t subnet) {
      this->subnet = subnet;
    }

    inline mesh24_subnet_t getSubnet() const {
      return subnet;
    }

    inline void setAddr(byte addr) {
      this->addr = addr;
    }

    inline byte getAddr() const {
      return addr;
    }
	
	inline RF24 getRadio() const {
      return radio;
    }
    ...
    void write(Mesh24Message& message, bool secured = false, bool isIntPadding = false) {
      return writeSecured(message, secured, isIntPadding);
    }
  ...
  private:
    bool readSecured(Mesh24Message& message) {
      ...
      if (type == MESH24_MSG_CREATE_SESSION_REQ) {
        ...
        return (sessionId & 0x80000000UL) || (sessionId & 0x8000UL) || sessionMgr.isValid(message.getFrom(), sessionId);
      }
    }
    ...
    void writeSecured(Mesh24Message& message, bool secured, bool isIntPadding = false) {
      if (message.getTo() == addr) {
        return;
      }
	...
	void addRandomPadding_int(Mesh24Message& message) {
      unsigned int sessionId = random(0x7fffL) | 0x8000UL;
      message.writePayload(-sizeof(sessionId), sessionId);
    }
